...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const tendersList = response.data.items || []
      setTenders(tendersList.slice(0, 5))

      // Calculate stats based on tender statuses
      const activeTenders = tendersList.filter((t: any) => 
        ['uploaded', 'extracting_criteria', 'criteria_ready'].includes(t.status)
      ).length
      const pendingEvals = tendersList.filter((t: any) => t.status === 'evaluating').length
      const needingReview = tendersList.filter((t: any) => t.status === 'completed').length
                ...
                >
                  ...
                  <div className="text-right">
                    <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${
                      ...
                      tender.status === 'evaluating' ? 'bg-warning-100 text-warning-700' :
                      tender.status === 'signed_off' ? 'bg-gray-100 text-gray-700' :
