...
export default function AuditLogsPage() {
  ...
  const loadAuditLogs = async () => {
    try {
      setLoading(true)
      const response = await auditAPI.getAuditLogs({ action: filterAction })
      ...
      setLogs([
        {
          ...
          action: 'upload',
          user: 'John Doe',
          description: 'Uploaded tender document',
          details: { file_name: 'tender_123.pdf', file_size: '2.5 MB' }
        },
        {
          id: 2,
          ...
          user: 'Jane Smith',
          description: 'Added evaluation criterion',
          details: { criterion_name: 'Financial Stability', weight: 'High' }
        },
        {
          id: 3,
          ...
          user: 'System',
          description: 'Started evaluation process',
          details: { job_id: 'eval_456', total_bidders: 3 }
        }
      ])
    } finally {
      setLoading(false)
  ...
  const actionTypes = [
    { value: 'upload', label: t('audit.actionTypes.upload') },
    { value: 'criteria_added', label: t('audit.actionTypes.criteria_added') },
    { value: 'evaluation_started', label: t('audit.actionTypes.evaluation_started') },
    { value: 'sign_off', label: t('audit.actionTypes.sign_off') }
  ]
                ...
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-4 mb-2">
                      <motion.span
                        className={`px-3 py-1 rounded-full text-sm font-semibold border ${getActionColor(log.action)}`}
                        whileHover={{ scale: 1.05 }}
                      >
                        {log.description}
                      </motion.span>
