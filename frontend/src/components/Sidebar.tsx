...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const currentTenderId = localStorage.getItem('current_tender') || 'demo-tender'

  const menuItems = [
    { icon: LayoutDashboard, label: t('nav.dashboard'), path: '/dashboard' },
    { icon: FileText, label: t('nav.tenders'), path: '/tenders' },
    { icon: CheckCircle, label: t('nav.evaluation'), path: `/evaluate/${currentTenderId}` },
    { icon: MessageSquare, label: t('nav.review'), path: '/review' },
    { icon: BarChart3, label: t('nav.reports'), path: `/reports/${currentTenderId}` },
